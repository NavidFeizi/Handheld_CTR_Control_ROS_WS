...
int main()
{
	...
	std::vector<double> dummy(4, 0);
	int counter = 0;
	std::vector<double> coefficients = {1.0 * M_PI / 180, 0.5E-3, 1.0 * M_PI / 180, 0.5E-3};
	// std::vector<double> coefficients = {1, 1, 1, 1};
	KeyboardInput inputHandler(dummy, coefficients, &counter);
	// Start the input handling thread in the main function
	...
	while (error > 1.00E-5 && counter < 1000UL)
	{
		auto startTime = std::chrono::high_resolution_clock::now();

		target = blaze::trans(blaze::row(Trajectory, counter));
		...
		double remainingTime_double = std::chrono::duration_cast<std::chrono::duration<double>>(elapsedTime).count();
		double remainingTime = std::chrono::duration_cast<std::chrono::duration<double>>(minimumSampleTime - elapsedTime).count();

		// Wait for the remaining time (50 milliseconds - elapsed time)
		if (remainingTime > 0)
		{
			std::this_thread::sleep_for(minimumSampleTime - elapsedTime);
		}
		...
		std::cout << "CTR Configuration: " << blaze::trans(CTR_robot.getConfiguration())
				  ...
				  << "Elapsed time = " << remainingTime_double << "[ms]\n"
				  << "--------------------------------------\n"
				  << std::endl;
	}
