...
namespace mathOp
{
	...
	{
		static constexpr double rad2deg = M_1_PI * 180.00;
	...
	{
		static constexpr double deg2rad = M_PI / 180.00;
	...
	{
		static constexpr double FOUR_PI = 4.00 * M_PI;
	...
	inline double shortestRotation(double currentAngle, double targetAngle)
	{
		...
		if (std::abs(clockwise) < std::abs(counterclockwise))
		{
			return congruentAngle(currentAngle + clockwise);
		}
		else
		{
			return congruentAngle(currentAngle - counterclockwise);
		}
	}
	...
	{
		// This implementation assumes the angles are passed in radians
