...
bool CTR::PowellDogLeg(blaze::StaticVector<double, 5UL> &initGuess)
{
	...
	auto readjustInitialGuesses = [](blaze::StaticVector<double, 5UL> &initial_guesses) -> void
	{
		blaze::subvector<2UL, 2UL>(initial_guesses) = blaze::map(blaze::subvector<2UL, 2UL>(initial_guesses), [](double d)
																 { return (!blaze::isfinite(d)) ? 0.00 : blaze::sign(d) * std::min(blaze::abs(d), 10.00); });
...
bool CTR::Modified_Newton_Raphson(blaze::StaticVector<double, 5UL> &initGuess)
{
	...
	*/
	...
	static constexpr size_t maxIter_D = 50UL;
	size_t iter_D = 0UL;
...
bool CTR::posCTRL(blaze::StaticVector<double, 5UL> &initGuess, const blaze::StaticVector<double, 3UL> &target, const double posTol)
{
	...
	blaze::diagonal(Kp) = 1.0000;
	blaze::diagonal(Ki) = 0.0005;
	blaze::diagonal(Kd) = 0.0001;
	...
	size_t iterJ = 0UL;
	static constexpr size_t maxIter_J = 50UL;
...
std::tuple<double, double, bool> CTR::constrainedPosCTRL(blaze::StaticVector<double, 5UL> &initGuess, const blaze::StaticVector<double, 3UL> &tgt_clx, const blaze::StaticVector<double, 3UL> &tgt_ee, const double posTol)
{
	...
	{
		blaze::subvector<2UL, 3UL>(initial_guesses) = blaze::map(blaze::subvector<2UL, 3UL>(initial_guesses), [](double d)
																 { return (!blaze::isfinite(d)) ? 0.00 : blaze::sign(d) * std::min(blaze::abs(d), 20.00); });
		// mb_x(0) = mb_y(0) = u3_z(0) = 0.00;
		initial_guesses[0UL] = initial_guesses[1UL] = initial_guesses[4UL] = 0.00;
	};
	...
	static constexpr double Qq = 10000, Qd = 500; // 10,000 | 500
...
std::tuple<blaze::StaticMatrix<double, 3UL, 6UL>, blaze::StaticMatrix<double, 3UL, 6UL>, blaze::StaticVector<double, 3UL>, blaze::StaticVector<double, 3UL>> CTR::computeAnatomicalJacobians(const blaze::StaticVector<double, 5UL> &initGuess, const double s)
{
	...
	blaze::StaticVector<double, 6UL> q_Original(this->m_q), q_Perturbed(this->m_q), q_Scaled(this->m_q);
	static constexpr double incr_scale = 1.00E-2, incr_floor = 5.00E-4; // 1.00E-3, incr_floor ==>> 1.00E-5 SEEM TO BE OPTIMAL;
...
// function that returns the point on the backbone that is closest to the renal calyx
double CTR::returnArcLength(const blaze::StaticVector<double, 3UL> &calyx)
{
	...
	auto dist = [&](const state_type &x) -> double
	{
		...
		const blaze::StaticVector<double, 3UL> subvec{x[8UL], x[9UL], x[10UL]};
	...
	const auto it = std::min_element(m_y.begin(), m_y.end(), [&](const auto &x, const auto &y)
...
{
	return {m_Tubes[0UL]->getStraightLen(), m_Tubes[1UL]->getStraightLen(), m_Tubes[2UL]->getStraightLen()};
}
...
std::tuple<blaze::HybridMatrix<double, 3UL, 1000UL, blaze::columnMajor>, blaze::HybridMatrix<double, 3UL, 1000UL, blaze::columnMajor>, blaze::HybridMatrix<double, 3UL, 1000UL, blaze::columnMajor>> CTR::getTubeShapes()
{
	...
	const blaze::StaticVector<double, 3UL> distalEnds(this->m_segment->getDistalEnds());
	...
	auto tubeEndIndex = [&](const size_t tube_index) -> size_t
	{
		auto it = std::lower_bound(this->m_s.begin(), this->m_s.end(), distalEnds[tube_index] - 1.00E-7);
		return std::distance(this->m_s.begin(), it);
	};

	const size_t distalIndex_Tb2 = tubeEndIndex(1UL);
	const size_t distalIndex_Tb3 = tubeEndIndex(2UL);

	const auto Tb_2 = blaze::submatrix(Tb_1, 0UL, 0UL, 3UL, distalIndex_Tb2 + 1UL);
	const auto Tb_3 = blaze::submatrix(Tb_1, 0UL, 0UL, 3UL, distalIndex_Tb3 + 1UL);
