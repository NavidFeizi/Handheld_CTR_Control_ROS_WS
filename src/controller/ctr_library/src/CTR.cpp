...
CTR::CTR(const std::array<std::shared_ptr<Tube>, 3UL> &Tb, blaze::StaticVector<double, 6UL> &q, double Tol, mathOp::rootFindingMethod method, double stageThickness) : m_accuracy(Tol), m_method(method), m_Tubes(Tb), m_beta(blaze::subvector<0UL, 3UL>(q)), m_q(q), m_stageThickness(stageThickness)
{
	...
	this->m_H_inv(0UL, 0UL) = this->m_H_inv(1UL, 1UL) = this->m_H_inv(2UL, 2UL) = this->m_H_inv(3UL, 3UL) = 1.00;
	...
	// establishing a connection with Open Slicer 3D
	this->m_connectionSlicer = std::make_shared<openIGTLink>(const_cast<char *>("localhost"), 18944);
	this->m_connectionSlicer->setInverseTransformation(this->m_H_inv);
}

// copy constructor
CTR::CTR(const CTR &rhs) : m_accuracy(rhs.m_accuracy), m_method(rhs.m_method), m_Tubes(rhs.m_Tubes), m_beta(rhs.m_beta), m_q(rhs.m_q),
						   m_theta_0(rhs.m_theta_0), m_e3(rhs.m_e3), m_H_inv(rhs.m_H_inv), m_h_0(rhs.m_h_0), m_y(rhs.m_y), m_s(rhs.m_s),
						   m_wf(rhs.m_wf), m_wm(rhs.m_wm), m_connectionSlicer(rhs.m_connectionSlicer), m_stageThickness(rhs.m_stageThickness)
{
	this->m_segment = std::make_unique<Segment>(this->m_Tubes, this->m_beta);
	this->m_stateEquations = std::make_unique<ODESystem>();
	this->m_stateObserver = std::make_unique<Observer>(this->m_y, this->m_s);
...
CTR::CTR(CTR &&rhs) noexcept
{
	...
	if (this != &rhs)
	{
		...
		this->m_connectionSlicer = std::move(rhs.m_connectionSlicer);
...
CTR &CTR::operator=(const CTR &rhs)
{
	...
	if (this != &rhs)
	{
		...
		this->objFuncStr = std::make_shared<objFuncData>();
		this->constrFuncStr = std::make_shared<constrFuncData>();
		this->m_connectionSlicer = rhs.m_connectionSlicer;
...
blaze::StaticMatrix<double, 3UL, 6UL> CTR::jacobian(const blaze::StaticVector<double, 5UL> &initGuess, const blaze::StaticVector<double, 3UL> &tipPos)
{
	...
	static constexpr double incr_scale = 1.00E-3, incr_floor = 5.00E-4; // 1.00E-3, incr_floor ==>> 1.00E-5 SEEM TO BE OPTIMAL;
...
bool CTR::PowellDogLeg(blaze::StaticVector<double, 5UL> &initGuess)
{
	...
	delta = 1.00;
	constexpr double eps1 = 1.00E-22, eps2 = 1.00E-22;
...
bool CTR::Levenberg_Marquardt(blaze::StaticVector<double, 5UL> &initGuess)
{
	...
	double rho, nu = 2.00, mu;
	constexpr double tau = 1.00E-3, e1 = 1.00e-18;
	bool found;
...
bool CTR::Newton_Raphson(blaze::StaticVector<double, 5UL> &initGuess)
{
	...
	while (!found && (k < k_max))
	{
		...
		blaze::subvector<2UL, 2UL>(initGuess) = blaze::map(blaze::subvector<2UL, 2UL>(initGuess), [](double d)
...
bool CTR::Modified_Newton_Raphson(blaze::StaticVector<double, 5UL> &initGuess)
{
	...
	*/
	...
	while (!found && (k < k_max) && !setupMethod())
	{
		k++;
...
std::tuple<double, double, bool> CTR::constrainedPosCTRL(blaze::StaticVector<double, 5UL> &initGuess, const blaze::StaticVector<double, 3UL> &tgt_clx, const blaze::StaticVector<double, 3UL> &tgt_ee, const double posTol)
{
	...
	size_t N_iter = 0UL;
	constexpr size_t maxIter = 250UL;
...
std::tuple<blaze::HybridMatrix<double, 3UL, 1000UL, blaze::columnMajor>, blaze::HybridMatrix<double, 3UL, 1000UL, blaze::columnMajor>, blaze::HybridMatrix<double, 3UL, 1000UL, blaze::columnMajor>> CTR::getTubeShapes()
{
	...
	{
		v = {this->m_y[col][8UL], this->m_y[col][9UL], this->m_y[col][10UL], 1.0000};
		v = this->m_H_inv * v;
...
void CTR::broadcastCalyxToSlicer(const blaze::StaticVector<double, 3UL> &calyx)
{
	this->m_connectionSlicer->broadcastCalyx(calyx);
}

// function that broadcasts a distal target to Slicer
void CTR::broadcastTargetToSlicer(const blaze::StaticVector<double, 3UL> &target)
{
	this->m_connectionSlicer->broadcastTarget(target);
}

// function that broadcasts the current CTR shape to 3D Slicer
void CTR::broadcastShapeToSlicer()
{
	auto [Tb1, Tb2, Tb3] = this->getTubeShapes();
	this->m_connectionSlicer->broadcastShapes(Tb1, Tb2, Tb3);
}

// function that broadcast ellipsoidal targets to Slicer
void CTR::broadcastEllipsoidToSlicer(const blaze::StaticMatrix<double, 3UL, 6UL> &Tgt)
{
	this->m_connectionSlicer->broadcastEllipsoidalTargets(Tgt);
}
...
const std::shared_ptr<openIGTLink> &CTR::getConnectionSlicer() const
{
	return this->m_connectionSlicer;
}