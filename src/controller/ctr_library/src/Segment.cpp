...
void Segment::recalculateSegments(const std::array<std::shared_ptr<Tube>, 3UL> &Tb, const blaze::StaticVector<double, 3UL> &beta)
{
	...
	// tolerance used to allow for small differences due to precision limitations
	...
	const auto it = std::unique(this->m_S.begin(), this->m_S.end(), compare);
	...
	const auto sBegin = this->m_S.begin();

	// determining the indexes correponding to tube transitions
	for (size_t i = 0UL; i < 3UL; ++i)
	{
		// transition points (straight -> curved sections): finds where curved section of the i-th starts
		it_b = std::lower_bound(sBegin, this->m_S.end(), this->m_len_curv[i] - TOLERANCE);

		// transition points -> distal ends: finds where i-th tube ends
		it_c = std::lower_bound(sBegin, this->m_S.end(), m_dist_end[i] - TOLERANCE);

		const size_t b = std::distance(sBegin, it_b);
		const size_t c = std::distance(sBegin, it_c);
