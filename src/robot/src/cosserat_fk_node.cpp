...
public:
  ...
  void actuate_timer_callback()
  {
    ...
    // std::cout << "fk_node --> q: " << blaze::trans(m_current_q) << std::endl;

    convergence = m_CTR_robot->actuate_CTR(m_initGuess, m_current_q);
    if (!convergence)
      std::cerr << "fk_node --> BVP did not converge" << std::endl;

    // std::cout << "fk_node --> tip position: " << blaze::trans(m_CTR_robot->getTipPos());
    std::flush(std::cout);

    const auto [Tb1, Tb2, Tb3] = m_CTR_robot->getTubeShapes(); // units are in [mm]
    ...
    std::cout << "fk_node --> first point of Tb1: [" 
        << Tb1(0, 0) << ", " 
        << Tb1(1, 0) << ", " 
        << Tb1(2, 0) << "]" << std::endl;
    std::cout << "fk_node --> tenth point of Tb1: [" 
        << Tb1(0, 9) << ", " 
        << Tb1(1, 9) << ", " 
        << Tb1(2, 9) << "]" << std::endl;
    std::cout << "fk_node --> second point of Tb1: [" 
        << Tb1(0, 69) << ", " 
        << Tb1(1, 69) << ", " 
        << Tb1(2, 69) << "]" << std::endl;
  ...
  std_msgs::msg::Float64MultiArray blazeToMultiArrayMsg(const blaze::HybridMatrix<double, 3UL, 1000UL, blaze::columnMajor> &mat)
  {
    ...
    const size_t rows = mat_row_major.rows();
    const size_t cols = mat_row_major.columns();

    // 3 rows × n cols → flattened into row-major
    msg.layout.dim.resize(2);
    msg.layout.dim[0UL].label = "rows";
    msg.layout.dim[0UL].size = rows;
    msg.layout.dim[0UL].stride = rows * cols;
    msg.layout.dim[1UL].label = "cols";
    msg.layout.dim[1UL].size = cols;
    msg.layout.dim[1UL].stride = cols;

    msg.data.resize(rows * cols);

    for (size_t i = 0; i < rows; ++i)    
      for (size_t j = 0; j < cols; ++j)      
        msg.data[i * cols + j] = mat_row_major(i, j) * 1.00E-3; // row-major order    
