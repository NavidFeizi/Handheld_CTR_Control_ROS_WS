...
public:
    ...
    // function that returns the number of backbone nodes used for shape inference
    ...
    // function that implements the inverse kinematics for the CTR under an
    // external tip force wf (the Jacobian and forward model are both evaluated
    // on the loaded/deflected robot)
