...
CTR_BackboneLengthObjective::CTR_BackboneLengthObjective(const ompl::base::SpaceInformationPtr &si, CTR& _ctr) : ompl::base::StateCostIntegralObjective(si, true), m_ctr(_ctr)
{}
...
ompl::base::Cost CTR_BackboneLengthObjective::stateCost(const ompl::base::State *s) const
{
    ...
    // std::cout << "Before actuation!" << std::endl;
    m_ctr.actuate_CTR(initialGuess, q);
    // std::cout << "After actuation! - tip position: " << blaze::trans(m_ctr.getTipPos()) << std::endl;
