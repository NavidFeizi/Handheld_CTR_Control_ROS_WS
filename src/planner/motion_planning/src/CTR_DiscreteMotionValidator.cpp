...
CTR_DiscreteMotionValidator::CTR_DiscreteMotionValidator(const ompl::base::SpaceInformationPtr &si, CTR& _ctr) : ompl::base::DiscreteMotionValidator(si), m_ctr(_ctr) 
{
...
{
    // std::cout << "CTR_DiscreteMotionValidator called!" << std::endl;
    ...
    while ((i < numSteps) && validConfiguration && validSequence)
    {
        ...
        if ((q_current[0UL] > q_current[1UL] - m_Clr) || (q_current[1UL] > q_current[2UL] - m_Clr))
            validSequence = false;
