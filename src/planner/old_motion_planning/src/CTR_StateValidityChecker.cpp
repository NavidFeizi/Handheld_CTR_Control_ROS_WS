...
CTR_StateValidityChecker::CTR_StateValidityChecker(const ompl::base::SpaceInformationPtr &si, std::shared_ptr<CTR> _ctr) : ompl::base::StateValidityChecker(si), ctr(_ctr) {}
...
bool CTR_StateValidityChecker::isValid(const ompl::base::State *state) const
{
    ...
    // lengths of the straight sections of the CTR component tubes
    static const blaze::StaticVector<double, 3UL> lengthStraight = ctr->getStraightLengthsOfTubes();

    // clearance between linear actuators (avoidance of collision between linear actuators)
    const double clr = ctr->getStageThickness();

    // (beta_1 >= -ls_1) AND (beta_1 <= beta_2 - clr)
    bool conditionTb1 = ((beta_1 >= -lengthStraight[0UL]) && (beta_1 <= beta_2 - clr)) ? true : false;

    // (beta_2 >= -ls_2) AND (beta_2 <= beta_3 - clr)
    bool conditionTb2 = ((beta_2 >= -lengthStraight[1UL]) && (beta_2 <= beta_3 - clr)) ? true : false;

    // (beta_3 >= -ls_3) AND (beta_3 <= 0.00 - clr)
    bool conditionTb3 = ((beta_3 >= -lengthStraight[2UL]) && (beta_3 <= 0.00)) ? true : false;
