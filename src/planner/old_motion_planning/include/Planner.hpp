...
public:
	...
	void plan(double runTime, optimalPlanner plannerType, planningObjective objectiveType, const std::string &tmepDirectory, const std::string &outputFile);
...
private:
	// CTR object for whom we want to plan for
	CTR m_robot;

	// reference to CTR objects to be used during state validity and motion validation checks
	std::shared_ptr<CTR> m_CTR_StateValidator, m_CTR_MotionValidator, m_CTR_ObjectiveFunction;  
