...
bool Planner<controlInputs>::planTwoPhase(const double runTime, optimalPlanner plannerType, const double phase2RangeFactor, const double phase2LinearStep)
{
	...
	{
		m_coordBound->setLow(i,  sst->values[i] - betaWindow);
		m_coordBound->setHigh(i, sst->values[i] + betaWindow);
	}
	m_space->as<ompl::base::RealVectorStateSpace>()->setBounds(*m_coordBound);

	this->resetGoalState(q_wp);
	this->plan(runTime * 0.5, plannerType, planningObjective::OBJECTIVE_REVJOINTS_ONLY);
	...
	// Phase 2 starts at the waypoint (β_start, α_goal).
	appendState(q_wp);
	...
	for (size_t step = 0; step < 2; ++step)
	{
		...
		{
			const bool isFinal = (n == nIncrements);
	...
	std::cout << "[planTwoPhase] Phase 2 (synchronized deployment): "
	          << phase2path.getStateCount() << " states (step=" << safeStep * 1.0e3 << " mm)." << std::endl;
...
void Planner<controlInputs>::analyzeSolution(bool dumpCSV, const std::string &csvPath)
{
	...
	if (printDiagnostics)
	{
		...
		std::cout << "Diagnostics: idx | path_frac | p | rev_base | scale | rev_scaled | rev_weighted | backbone_raw | prismatic_weighted | backbone_coupled | mismatch | gating | alignment_progress | req_alignment | deficit | ordering_penalty | composite | max_angle_err_deg" << std::endl;
	}
	...
	for (std::size_t i = 0; i < N; ++i)
	{
		...
		if (printDiagnostics)
		{
			std::cout << i << " | " << pathFrac << " | " << p << " | " << rev_base << " | " << scale << " | " << rev_scaled << " | " << revoluteWeighted << " | " << backbone_raw << " | " << prismaticWeighted << " | " << backbone_coupled << " | " << mismatch << " | " << gating << " | " << alignmentProgress << " | " << reqAlignment << " | " << deficit << " | " << lagPenalty << " | " << composite << " | " << max_err_deg << std::endl;
		}

		if (csv.is_open())
			csv << i << "," << pathFrac << "," << p << "," << rev_base << "," << scale << "," << rev_scaled << "," << revoluteWeighted << "," << backbone_raw << "," << prismaticWeighted << "," << backbone_coupled << "," << mismatch << "," << gating << "," << alignmentProgress << "," << reqAlignment << "," << deficit << "," << lagPenalty << "," << composite << "," << max_err_deg << "\n";
	}
...
ompl::base::OptimizationObjectivePtr Planner<controlInputs>::RevoluteJointsAndBackboneLengthObjective(const std::array<double, 2UL> &goal)
{
	...
	public:
		...
		ompl::base::Cost motionCost(const ompl::base::State *s1, const ompl::base::State *s2) const override
		{
			...
			return ompl::base::Cost(base.value() + w_transition * delta_p * normAngleErr1 * normAngleErr1);
		}
	...
	private:
		...
		ompl::base::SpaceInformationPtr m_si;
		PINNs<controlInputs> &m_ctrModel;
...
ompl::base::OptimizationObjectivePtr Planner<controlInputs>::getBackboneLengthObjective()
{
	...
	obj->setCostToGoHeuristic([this](const ompl::base::State *state, const ompl::base::Goal *goal) -> ompl::base::Cost
		...
		try
		{
			this->m_CTR_model.getPosDistal(q_cur, tip_cur);
		}
		...
		if (goal->hasType(GoalType::GOAL_STATE))
		{
			...
			if (gstate)
			{
				...
				try
				{
					this->m_CTR_model.getPosDistal(qg, tip_goal);
				}
				catch (...)
				{
					return baseline;
