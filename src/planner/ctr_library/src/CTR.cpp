...
CTR::CTR(const std::array<std::shared_ptr<Tube>, 3UL> &Tb, blaze::StaticVector<double, 6UL> &q, double Tol, mathOp::rootFindingMethod method, double stageThickness) : m_accuracy(Tol), m_method(method), m_Tubes(Tb), m_beta(blaze::subvector<0UL, 3UL>(q)), m_q(q), m_stageThickness(stageThickness)
{
	...
	blaze::diagonal(this->m_H_inv) = 1.00;
...
CTR::CTR(const CTR &rhs) : m_accuracy(rhs.m_accuracy), m_method(rhs.m_method), m_Tubes(rhs.m_Tubes), m_beta(rhs.m_beta),
						   ...
						   m_h_0(rhs.m_h_0), m_y(rhs.m_y), m_s(rhs.m_s), m_wf(rhs.m_wf), m_wm(rhs.m_wm), m_stageThickness(rhs.m_stageThickness)
{
	this->m_segment = std::make_unique<Segment>(this->m_Tubes, this->m_beta);
	this->m_stateEquations = std::make_unique<ODESystem>();
	this->m_stateObserver = std::make_unique<Observer>(this->m_y, this->m_s);
...
CTR::CTR(CTR &&rhs) noexcept
{
	...
	if (this != &rhs)
	{
		...
		this->m_H_inv = std::move(rhs.m_H_inv);
...
{
	const blaze::StaticVector<double, 3UL> uz_0 = {initGuess[2UL], initGuess[3UL], initGuess[4UL]};
	// alpha1_0 =  alpha_1 - beta_1 * uz_1(0)
	const double alpha1_0 = this->m_q[3UL] - this->m_beta[0UL] * uz_0[0UL];

	// clearing the observer's containers
	if (!this->m_s.empty())
	{
		const size_t len = this->m_y.size();
		this->m_y.clear();
		this->m_s.clear();
		this->m_y.reserve(len);
		this->m_s.reserve(len);
	}
...
blaze::StaticVector<double, 5UL> CTR::ODESolver(const blaze::StaticVector<double, 5UL> &initGuess)
{
	...
	boost::numeric::odeint::adaptive_adams_bashforth_moulton<5UL,
															 State,
															 double,
															 State,
															 double,
	...
	constexpr double ds = 5.00E-4;

	// Tolerance parameters for the adaptive stepper
	constexpr double abs_tol = 1.00E-8; // absolute tolerance means "only care about the solution if its value is larger than this"
	constexpr double rel_tol = 1.00E-8; // relative tollerance means how many digits
	...
	for (size_t seg = 0UL; seg < len_seg; ++seg)
	{
	...
	// Residue vector due to infringment of the distal boundary conditions || Residue = [ mb_x(L) - Wm_x, mb_y(L) - Wm_y, k1 * u1_z(L) - e3' * R1(L)' * wm, u2_z(L), u3_z(L) ]
...
blaze::StaticMatrix<double, 5UL, 5UL> CTR::jac_BVP(const blaze::StaticVector<double, 5UL> &initGuess, const blaze::StaticVector<double, 5UL> &residue)
{
	...
	static constexpr double incr_scale = 1.00E-7, incr_floor = 1.00E-9; // 1.00E-7, incr_floor = 1.00E-9 ==>> SEEM TO BE OPTIMAL;
...
blaze::StaticMatrix<double, 3UL, 6UL> CTR::jacobian(const blaze::StaticVector<double, 5UL> &initGuess, const blaze::StaticVector<double, 3UL> &tipPos)
{
	...
	constexpr double incr_scale = 1.00E-3, incr_floor = 5.00E-4; // 1.00E-7, incr_floor ==>> 1.00E-9 SEEM TO BE OPTIMAL;
...
bool CTR::PowellDogLeg(blaze::StaticVector<double, 5UL> &initGuess)
{
	...
	blaze::StaticMatrix<double, 5UL, 5UL, blaze::columnMajor> J;
...
bool CTR::Levenberg_Marquardt(blaze::StaticVector<double, 5UL> &initGuess)
{
	...
	constexpr blaze::IdentityMatrix<double> I(5UL);
...
bool CTR::Newton_Raphson(blaze::StaticVector<double, 5UL> &initGuess)
{
	...
	while (!found && (k < k_max))
	{
		...
		blaze::subvector<2UL, 3UL>(initGuess) = blaze::map(blaze::subvector<2UL, 3UL>(initGuess), [](double d)
														   { return (std::isnan(d)) ? 0.00 : blaze::sign(d) * std::min(std::fabs(d), 25.00); });
		// For the hand-held CTR, u1_x(0) = u1_y(0) = u3_z(0) = 0.00
		initGuess[0UL] = initGuess[1UL] = 0.00;
...
bool CTR::posCTRL(blaze::StaticVector<double, 5UL> &initGuess, const blaze::StaticVector<double, 3UL> &target, const double posTol)
{
	...
	static constexpr double deltaBar = 0.00;

	// lengths of straight sections of the CTR tubes
	const blaze::StaticVector<double, 3UL> L = {
		this->m_Tubes[0UL]->getTubeLength(),
		this->m_Tubes[1UL]->getTubeLength(),
		this->m_Tubes[2UL]->getTubeLength()};

	const blaze::StaticVector<double, 3UL> ls = {
		this->m_Tubes[0UL]->getStraightLen(),
		this->m_Tubes[1UL]->getStraightLen(),
		this->m_Tubes[2UL]->getStraightLen()};
	...
	static constexpr size_t maxIter = 1500UL;
	...
	while ((dist2Tgt > posTol) && (N_itr < maxIter))
	{
		...
		betaMin[1UL] = std::max({-ls[1UL] + deltaBar, this->m_beta[0UL] + m_stageThickness, L[2UL] + this->m_beta[2UL] - L[1UL]});
...
std::tuple<double, double, bool> CTR::constrainedPosCTRL(blaze::StaticVector<double, 5UL> &initGuess, const blaze::StaticVector<double, 3UL> &tgt_clx, const blaze::StaticVector<double, 3UL> &tgt_ee, const double posTol)
{
	...
	constexpr double Qq = 10000, Qd = 500; // 10,000 | 500
	...
	static constexpr blaze::IdentityMatrix<double> I(6UL), I_damp(3UL);

	// computing the point on the CTR backbone closest to the calyx
	s = this->returnArcLength(tgt_clx);

	// populating the equality constraint matrix A := [J_aug(6x6) -I(6x6)]
	...
	{
		const blaze::StaticVector<double, 12UL, blaze::columnVector> xOpt = {x[0UL], x[1UL], x[2UL], x[3UL], x[4UL], x[5UL], x[6UL], x[7UL], x[8UL], x[9UL], x[10UL], x[11UL]};
		...
		if (!grad.empty())
		{
			const blaze::StaticVector<double, 12UL> gradFunc = data->Q * xOpt;
	...
	auto constraint = [](unsigned m, double *result, unsigned n, const double *x, double *grad, void *f_data) -> void
	{
		...
		if (grad != nullptr)
		{
			const blaze::StaticMatrix<double, 12UL, 6UL> gradConstr = blaze::trans(data->A);
		...
		const blaze::StaticVector<double, 6UL> h_eq = data->A * xOpt - data->xdot; // equality constraint
		result[0UL] = h_eq[0UL];
		result[1UL] = h_eq[1UL];
		result[2UL] = h_eq[2UL];
		result[3UL] = h_eq[3UL];
	...
	size_t N_iter = 0UL;
	constexpr size_t maxIter = 750UL;

	qMin[3UL] = qMin[4UL] = -M_PI;
	qMax[3UL] = qMax[4UL] = M_PI;
	// translation & rotation of outermost tube -- unactuated
	qMin[2UL] = qMin[5UL] = 0.00;
	qMax[2UL] = qMax[5UL] = 0.00;
	...
	while ((posError > posTol) && (N_iter < maxIter))
	{
		...
		if (!status)
		{
			...
			if (!status)
			{
				// std::cout << "Failed to converge: " << __PRETTY_FUNCTION__ << std::endl;
				status = this->actuate_CTR(initGuessMin, q_min);
...
std::tuple<blaze::StaticMatrix<double, 3UL, 6UL>, blaze::StaticMatrix<double, 3UL, 6UL>, blaze::StaticVector<double, 3UL>, blaze::StaticVector<double, 3UL>> CTR::computeAnatomicalJacobians(const blaze::StaticVector<double, 5UL> &initGuess, const double s)
{
	...
	blaze::StaticVector<double, 6UL> q_Original(this->m_q), q_Perturbed(this->m_q), q_Scaled(this->m_q);
	constexpr double incr_scale = 1.00E-3, incr_floor = 5.00E-4; // 1.00E-3, incr_floor ==>> 1.00E-5 SEEM TO BE OPTIMAL;
...
void CTR::setHomogeneousTransformation(const blaze::StaticMatrix<double, 4UL, 4UL> &H)
{
	// computes the inverse (CTR -> patient) homogeneous transformation
	this->m_H_inv = mathOp::pInv(H);
}
...
{
	// Use blaze::StaticVector to store the subvector for efficiency
	blaze::StaticVector<double, 3UL> subvec;

	// Lambda comparator: compute the point on the backbone closest to the renal calyx
	auto dist = [&](const state_type &x) -> double
	{
		subvec[0UL] = x[8UL];
		subvec[1UL] = x[9UL];
		subvec[2UL] = x[10UL];
		return blaze::sqrNorm(subvec - calyx); // Use squared norm to avoid unnecessary sqrt calculations
	};

	// Find the iterator to the minimum element using the distance function
	auto it = std::min_element(m_y.begin(), m_y.end(), [&](const auto &x, const auto &y)
							   { return dist(x) < dist(y); });

	// Compute the index of the nearest neighbor on the array
	size_t idx = std::distance(m_y.begin(), it);
...
blaze::StaticVector<double, 3UL> CTR::getStraightLengthsOfTubes()
{
	blaze::StaticVector<double, 3UL> lengthStraight = {m_Tubes[0UL]->getStraightLen(), m_Tubes[1UL]->getStraightLen(), m_Tubes[2UL]->getStraightLen()};

	return lengthStraight;
}
...
std::tuple<blaze::HybridMatrix<double, 3UL, 1000UL, blaze::columnMajor>, blaze::HybridMatrix<double, 3UL, 1000UL, blaze::columnMajor>, blaze::HybridMatrix<double, 3UL, 1000UL, blaze::columnMajor>> CTR::getTubeShapes()
{
	...
	blaze::StaticVector<double, 4UL> v = {0.00, 0.00, 0.00, 1.00};
	blaze::StaticVector<double, 3UL> distalEnds(this->m_segment->getDistalEnds());

	for (size_t col = 0UL; col < Tb_1.columns(); ++col)
	{
		v[0UL] = this->m_y[col][8UL];
		v[1UL] = this->m_y[col][9UL];
		v[2UL] = this->m_y[col][10UL];
		v = this->m_H_inv * v;
		blaze::column(Tb_1, col) = blaze::subvector<0UL, 3UL>(v) * 1.00E3;
	}

	auto tubeEndIndex = [&](size_t tube_index) -> size_t
	{
		auto it = std::lower_bound(this->m_s.begin(), this->m_s.end(), distalEnds[tube_index] - 1.00E-7);
		return std::distance(this->m_s.begin(), it);
	};

	size_t distalIndex_Tb2 = tubeEndIndex(1UL);
	size_t distalIndex_Tb3 = tubeEndIndex(2UL);
	...
	return std::make_tuple(std::move(Tb_1), std::move(Tb_2), std::move(Tb_3));
}
...
std::tuple<std::vector<double>, std::vector<double>, std::vector<double>> CTR::getShape()
{
	...
	// Fill the vectors with the relevant elements from m_y
...
// method that aligns the precurvatures of the CTR compnent tubes
void CTR::alignCurvatures()
{
	blaze::subvector<3UL, 3UL>(this->m_q) = 0.00;
	this->setConfiguration(this->m_q);
}

// set the thickness of the linear stages of the robot (used for actuation collision avoidance)
void CTR::setStageThickness(const double stageThickness)
{
	this->m_stageThickness = stageThickness;
}

// getter method for returning the thickness of the linear stages of the robot (used for actuation collision avoidance)
double CTR::getStageThickness()
{
	return this->m_stageThickness;
}
...
// function that returns the maximum and minimum torsion on the CTR assembly
std::tuple<double, double, double, double, double, double> CTR::getMinMaxTubeTorsions()
{
	// min and max torsions along the innermost tube -- Tube 1
	auto minmax_theta1 = std::minmax_element(this->m_y.begin(), this->m_y.end(),
											 [](const auto &a, const auto &b)
											 { return a[2UL] < b[2UL]; });
	// min and max torsions along the intermediate tube -- Tube 2
	auto minmax_theta2 = std::minmax_element(this->m_y.begin(), this->m_y.end(),
											 [](const auto &a, const auto &b)
											 { return a[3UL] < b[3UL]; });
	// min and max torsions along the outermost tube -- Tube 3
	auto minmax_theta3 = std::minmax_element(this->m_y.begin(), this->m_y.end(),
											 [](const auto &a, const auto &b)
											 { return a[4UL] < b[4UL]; });

	// Extract the specific elements from the iterators and construct the tuple
	return std::make_tuple((*minmax_theta1.first)[2UL], (*minmax_theta1.second)[2UL],
						   (*minmax_theta2.first)[3UL], (*minmax_theta2.second)[3UL],
						   (*minmax_theta3.first)[4UL], (*minmax_theta3.second)[4UL]);
}