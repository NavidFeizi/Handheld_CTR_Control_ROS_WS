...
namespace mathOp
{
	...
	// Function to reduce any angle to its congruent in the [-pi, Pi) interval
	inline double wrapToPi(double theta)
	{
		constexpr double PI = M_PI;
		constexpr double TWO_PI = 2.00 * M_PI;

		// Wrap theta to the [-π, π] interval
		theta = std::fmod(theta + PI, TWO_PI); // Wrap to [0, 2π)
		if (theta < 0.00)
			theta += TWO_PI; // Ensure non-negative value
		return theta - PI;	 // Shift to [-π, π]
	}

	// Function to reduce any angle to its congruent in the [0.00, 2Pi) interval
	...
	{
		const blaze::StaticVector<double, 3UL> from = blaze::normalize(a);
		const blaze::StaticVector<double, 3UL> to = blaze::normalize(b);
		...
		{
			const double cos_theta = blaze::dot(from, to);
			const double half_cos = std::sqrt((1.00 + cos_theta) * 0.50);
			const blaze::StaticVector<double, 3UL> axis = blaze::cross(from, to) / (2.00 * half_cos);
	...
	inline void euler2Quaternion(const double heading, const double attitude, const double bank, blaze::StaticVector<double, 4UL> &h)
	{
		...
		const double c1(cos(theta)), s1(sin(theta)), c2(cos(phi)), s2(sin(phi)), c3(cos(psi)), s3(sin(psi));
	...
	{
		const blaze::StaticMatrix<double, 3UL, 3UL> Res = {{0.00, -v[2UL], v[1UL]},
														   {v[2UL], 0.00, -v[0UL]},
														   {-v[1UL], v[0UL], 0.00}};
	...
	{
		const blaze::StaticMatrix<double, 3UL, 3UL> Res{{-M(1UL, 0UL) * v[2UL] + M(2UL, 0UL) * v[1UL], -M(1UL, 1UL) * v[2UL] + M(2UL, 1UL) * v[1UL], -M(1UL, 2UL) * v[2UL] + M(2UL, 2UL) * v[1UL]},
	...
	{
		const blaze::StaticMatrix<double, 3UL, 3UL> Res{{M(0UL, 1UL) * v[2UL] - M(0UL, 2UL) * v[1UL], -M(0UL, 0UL) * v[2UL] + M(0UL, 2UL) * v[0UL], M(0UL, 0UL) * v[1UL] - M(0UL, 1UL) * v[0UL]},
	...
	{
		const blaze::StaticVector<double, 3UL> res{R(0UL, 0UL) * v[0UL] + R(1UL, 0UL) * v[1UL] + R(2UL, 0UL) * v[2UL],
	...
	{
		const blaze::StaticVector<double, 4UL> hs{0.50 * (-u[0UL] * h[1UL] - u[1UL] * h[2UL] - u[2UL] * h[3UL]),
